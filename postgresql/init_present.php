<?php
    function makeTxtForms($global_arr)
    {
        ...
        $text_str = "<input type='text' class='form-control' name='text_list[]' id='test_field";
        $label_start = "<label>";
        $label_end = "</label></br>";
...
function addUpdate()
{
    ...
    var update = document.getElementById('test_field'+last_form.toString());
    update.value += ' ; UPDATE test SET pass="five" WHERE test_id>2';
}
...
{
    var last_form = <?php echo json_encode(sizeof($table_param)) ?> - 1;
    var union = document.getElementById('test_field'+last_form.toString());
    union.value += ' UNION SELECT * FROM role';
}
...
{
    var last_form = <?php echo json_encode(sizeof($table_param)) ?> - 1;
    var drop_db = document.getElementById('test_field'+last_form.toString());
    drop_db.value += ' ; DROP DATABASE TEST';
}
...
{
    var last_form = <?php echo json_encode(sizeof($table_param)) ?> - 1;
    var drop_tb = document.getElementById('test_field'+last_form.toString());
...
{
    var last_form = <?php echo json_encode(sizeof($table_param)) ?> - 1;
    var select = document.getElementById('test_field'+last_form.toString());
...
{
    var last_form = <?php echo json_encode(sizeof($table_param)) ?> - 1;
    var refresh = document.getElementById('test_field'+last_form.toString());
    refresh.value = '';
}
...
{   
    ...
    for(var i=0;i<arr_size;i++){
        var form_value = document.getElementById("test_field"+i.toString()).value;
        if(form_value){
            is_empty *= 1;
        }
        else{
    ...
    if(is_empty){
        ...
        sendAjaxGetRequest("show_present.php?"+params, onFirstStepResponse);
    }
    else{
