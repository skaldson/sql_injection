<?php
    ...
    $restrict = [';'];
    $sql_operators = ['SELECT', 'DROP', 'OR', 'AND', 'SHOW', 'UPDATE', 'UNION'];
    $compare_operator = ['>', '<', '!', '='];

    $db_name = "kolenapost";
    $dbh=MySQLDatabase::connect($db_name,"root","");
    $stmt_db = $dbh->prepare("USE {$db_name}");
    $stmt_db->execute();
    ...
    $query = $query.' * '.$query_from.$db_name.'.'.$table_name.$query_condition.$condition_kernel;
    ...
    for($i=0;$i<sizeof($query_array);$i++)
    {
        ...
        if($query_result['drop'])
        {
            ...
            exec("sudo mysql < /home/skaldson/Downloads/test_table.sql");
            exec('sudo mysql < /home/skaldson/python_skald/kolenapost/sql_scr.sql');
        }
